...
bool CScriptHandler::HandleCommand(const char* sCommand, TargaImage*& pImage)
{
    ...
    switch (command)
    {
        case LOAD:
        {
            ...
            bResult = (pImage = TargaImage::Load_Image(sFilename)) != NULL;//OPERATION 1: load image
        ...
        case SAVE:
        {
            ...
            bResult =  bParsed && pImage->Save_Image(sFilename);//OPERATION 2: save image
            break;
        }// SAVE

        case RUN:
        {
            bResult = HandleScriptFile(strtok(NULL, c_sWhiteSpace), pImage);//OPERATION 4: save script
            break;
        }// RUN

        case GRAY:
        {
            bResult = pImage->To_Grayscale();//OPERATION 5: grayscale
            break;
        }// GREY

        case QUANT_UNIF:
        {
            bResult = pImage->Quant_Uniform();//OPERATION 6: Uniform Quantization
            break;
        }// QUANT_UNIF

        case QUANT_POP:
        {
            bResult = pImage->Quant_Populosity();//OPERATION 7: Populosity
            break;
        }// QUANT_POP

        case DITHER_THRESH:
        {
            bResult = pImage->Dither_Threshold();//OPERATION 8: Naive Threshold Dithering            
            break;
        }// DITHER_THRESH

        case DITHER_RAND:
        {
            bResult = pImage->Dither_Random();//OPERATION 10: Random Dithering
            break;
        }// DITHER_RAND

        case DITHER_FS:
        {
            bResult = pImage->Dither_FS();//OPERATION 12: Floyd-Steinberg Dithering
            break;
        }// DITHER_FS

        case DITHER_BRIGHT:
        {
            bResult = pImage->Dither_Bright();//OPERATION 9: Brightness Preserving Threshold Dithering
            break;
        }// DITHER_BRIGHT
        
        case DITHER_CLUSTER:
        {
            bResult = pImage->Dither_Cluster();//OPERATION 11: Clustered Dithering
            break;
        }// DITHER_CLUSTER
        
        case DITHER_COLOR:
        {
            bResult = pImage->Dither_Color();//OPERATION 13: Color Floyd-Steinberg Dithering
            break;
        }// DITHER_COLOR

        case FILTER_BOX:
        {
            bResult = pImage->Filter_Box();//OPERATION 14: Box Filter
            break;
        }// DITHER_BOX

        case FILTER_BARTLETT:
        {
            bResult = pImage->Filter_Bartlett();//OPERATION 15: Bartlett Filter
            break;
        }// DITHER_BARTLETT

        case FILTER_GAUSS:
        {
            bResult = pImage->Filter_Gaussian();//OPERATION 16: Gaussian Filter
            break;
        }// FILTER_GUASS
        ...
        case FILTER_GAUSS_N:
        {
            ...
            bResult = pImage->Filter_Gaussian_N(N);//OPERATION 17: Arbitrary-Size Gaussian Filter
            break;
        }// FILTER_GUASS_N
        ...
        case NPR_PAINT:
        {
            bResult = pImage->NPR_Paint();//OPERATION 22: NPR Paint
            break;
        }// NPR_PAINT
        ...
        case HALF:
        {
            bResult = pImage->Half_Size();//OPERATION 18: Half Size
            break;
        }// HALF

        case DOUBLE:
        {
            bResult = pImage->Double_Size();//OPERATION 19: DOUBLE Size
            break;
        }// DOUBLE
        ...
        case DIFF:
        {
            ...
            bResult = pNewImage && pImage->Difference(pNewImage);//OPERATION 3: subtrct image
            delete pNewImage;
            break;
        }// DIFF
        ...
        case ROTATE:
        {
            ...
            if (!sAngle || !(angle = (float)atof(sAngle)))
            {
                ...
                bResult = pImage->Rotate(angle);//OPERATION 21: Arbitrary Rotation
            break;
